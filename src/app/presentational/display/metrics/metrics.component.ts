...
export class MetricsComponent implements OnInit {
  ...
  importanceDifficultyUrgencyConvert(format, number) {
  ...
  stringifyVariations(v: number[]): string {
    return `${this.importanceDifficultyUrgencyConvert('difficulty', v[0])},
  ${this.importanceDifficultyUrgencyConvert('importance', v[1])},
   ${this.importanceDifficultyUrgencyConvert('urgency', v[2])}`
  }


  currentMonth(dateToCheck) {
    return moment.utc(new Date(dateToCheck)).isSame(new Date(), 'month'); //true if dates are in the same month
  }

  currentWeek(dateToCheck) {

    return moment.utc(new Date(dateToCheck)).isSame(new Date(), 'week'); //true if dates are in the same week
  }

  lastWeek(dateToCheck) {
    const sevenDaysAgo = this.atimeAgo(new Date(), 7)
    return moment.utc(new Date(dateToCheck)).isSame(sevenDaysAgo, 'week'); //true if dates are in the same week
  }
